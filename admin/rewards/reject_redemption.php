...
if($redemption && strtolower($redemption['status'] ?? '') === 'pending'){
  ...
  $stmt = $db->prepare("UPDATE redemptions SET status='rejected', updated_at=NOW() WHERE id=?");
  $stmt->bind_param('i', $id);
  $stmt->execute();
  $stmt->close();
  ...
  if($redemption['voucher_code']){
    ...
    if($voucher_check && $voucher_check->num_rows > 0){
      ...
      $voucher_stmt = $db->prepare("UPDATE vouchers SET status='rejected' WHERE `{$voucher_code_col}`=?");
      if($voucher_stmt){
  ...
  header('Location: redemptions.php?msg='.urlencode('Redemption rejected.'));
} else {
